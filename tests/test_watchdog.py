import importlib.util, os, signal, stat, subprocess, tempfile, time
...
with tempfile.TemporaryDirectory(prefix='watchdog-test-') as tmp:
    ...
    source.write_text('#include <unistd.h>\n#include <stdio.h>\nint main(void) { pid_t child=fork(); if(child==0){execl("/bin/sleep","sleep","60",(char*)0);_exit(1);} printf("%d\\n",child);fflush(stdout);for(;;)pause();}\n')
    ...
    try:
        ...
        watcher = subprocess.Popen([str(ROOT / 'build' / 'watchdog'), '--test-target', str(fixture)], stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, text=True)
